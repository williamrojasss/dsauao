...
public class DSA {
    ...
    public static void main(String [] args) throws NoSuchAlgorithmException, IOException {
        ...
        switch (args [0])
        {
            ...
            case "-key":
             switch (args [1]){
                    case "-h":
                    System.out.println(DSA.mensajes("-key -h"));
                    break;
                    default:
                      // Args 1 - Nombre Llave
            ...
            case "-sign":{
                switch (args [1]){
                    case "-h":
                    System.out.println(DSA.mensajes("-sign -h"));
                    break;
                    default:
                    // Args 1 - Argumento Archivo para firmar.
                    // Args 2 - Nombre Llave Privada.
                    Firma(args [2],args[1]);
                    break;
                }
            ...
            case "-verify":{
                 switch (args [1]){
                    case "-h":
                    System.out.println(DSA.mensajes("-verify -h"));
                    break;
                    default:
                    // Args 1 - Argumento Archivo para firmar.
                    // Args 2 - Nombre llave publica.
                    // Args 3 - Valor R de firma.
                    // Args 4 - Valor S de firma.
                    Verificar(args[2],args[1],args[3],args[4]);
                    break;
                }
            }
            break;
    ...
    public static String KeyFile(String Key) throws FileNotFoundException, IOException {
        String cadena;
        String c = "";
    ...
    public static BigInteger hash_sha1(String File) throws FileNotFoundException, IOException{
    ...
    public static void Firma(String keyPri, String File) throws IOException{
    String P = "13232376895198612407547930718267435757728527029623408872245156039757713029036368719146452186041204237350521785240337048752071462798273003935646236777459223";
    String Q = "857393771208094202104259627990318636601332086981";
    String G = "5421644057436475141609648488325705128047428394380474376834667300766108262613900542681289080713724597310673074119355136085795982097390670890367185141189796";
    keyPri = KeyFile(keyPri);
    BigInteger p = new BigInteger(P);
    BigInteger q = new BigInteger(Q);
    BigInteger g = new BigInteger(G);
    ...
    BigInteger mhash = hash_sha1(File);
    
    BigInteger s2 = r.multiply(x);
    BigInteger s3 = mhash.add(s2);
    BigInteger s = kinverse.multiply(s3);
    System.out.println(r);
    System.out.println(s);
    
    String R = r.toString();
        System.out.println("R  "+R);
    String encodedkeyR = Base64.getEncoder().encodeToString(R.getBytes());
        System.out.println("Base64keyR "+encodedkeyR);
    String f2 = "R"+".sign";
    FileOutputStream out = new FileOutputStream(f2);
    out = new FileOutputStream(f2);    
    byte[] bytesKeyR = encodedkeyR.getBytes();    
    out.write(bytesKeyR);
    out.close();    
    
    String S = s.toString();
        System.out.println("S  "+S);
    String encodedkeyS = Base64.getEncoder().encodeToString(S.getBytes());
        System.out.println("Base64keyPub "+encodedkeyS);
    f2 = "S"+".sign";
    out = new FileOutputStream(f2);    
    byte[] bytesKeyS = encodedkeyS.getBytes();    
    out.write(bytesKeyS);
    out.close();    
    
    
    }
    public static boolean Verificar(String keyPub, String File, String R, String S) throws IOException{
    ...
    keyPub = KeyFile(keyPub);
    System.out.println(keyPub);
    
    byte[] decodedkeyPub = Base64.getDecoder().decode(keyPub);
    String decodedkeyPubS = new String(decodedkeyPub);    
    System.out.println("decodedkeyPub "+decodedkeyPubS);
    BigInteger y = new BigInteger(decodedkeyPubS);
    
    S = KeyFile(S);
    System.out.println("S:"+S);
    byte[] decodedkeyS = Base64.getDecoder().decode(S);
    String decodkeyS = new String(decodedkeyS);    
    System.out.println("decodedS "+decodkeyS);
    BigInteger s = new BigInteger(decodkeyS);
    
    R = KeyFile(R);
    System.out.println("R:"+R);
    byte[] decodedkeyR = Base64.getDecoder().decode(R);
    String decodkeyR = new String(decodedkeyR);    
    System.out.println("decodedR "+decodkeyR);
    BigInteger r = new BigInteger(decodkeyR);
    
    
    BigInteger w = s.modInverse(q);
    BigInteger u1 = mhash.multiply(w);
    u1 = u1.mod(q);
    ...
    /*
    BigInteger vg = g.pow(u1.intValue());
    BigInteger vy = y.pow(u2.intValue());
    BigInteger v = vg.multiply(vy);
    v = v.mod(p);
    v = v.mod(q);
    ...
    public static String mensajes(String args){
        ...
        switch (args){
                ...
                mensaje = "-----------------------------------------------------------------------------------------------------------------------------\n"
                        ...
                        + "\nSe ha programado dos maneras para la firma digital con DSA, una implementación manual donde se replica el algoritmo y  \n"
                        + "\nutilizando la clase java.security que trae la generación de firmas digitales con el algoritmo DSA.\n"
                        + "sintaxis manual: java DSA <argumento>\n"
                        + "<argumento>:\n"
                        + "     -key        Para genrear el par llaves privada y publica.\n"
                        + "     -sign       Para generar la firma digital de un archivo.\n"
                        + "     -verify     Para verificar la autenticidad e integridad de un archivo.\n\n"
                        + "sintaxis automatico: java DSA <argumento>\n"
                        + "<argumento>:\n"
                ...
                case "-key -h":
                    mensaje = "----------------------------------------------------------------------------------------------------------------------------\n"
                            + "                          GENERACION DE PAR LLAVES PRIVADA & PUBLICA PARA ALGORITMO DSA\n"
                            + "-----------------------------------------------------------------------------------------------------------------------------\n"
                            + "Esta es la primera parte del algoritmo DSA, generar el par de llaves criptograficas publica y privada\n"
                            + "se generarán dos achivos .private y .public con la ejecución de esta parte del algoritmo y dos archivos base64_keyname codificados"
                            + "Las llaves cumplen con los requisitos del algoritmo DSA:\n"
                            + "Generar un numero primo p de longitud 512 bits \n"
                            + "Generar un numero primo q de longitud 160 bits tal que p-1 mod q = 0 \n"
                            + "Un numero g tal que g^q mod p == 1 \n"
                            + "Un numero x (clave privada), número aleatorio que cumple con q > x \n"
                            + "Un numero y (clave publica), que cumple con g^x mod p == y \n\n"
                            + "Sintaxis: java DSA.java -key <nombrellave>\n"
                            + "<nombrellave>:\n"
                            + "     <nombrellave>:     Nombre para los archivos de clave publica y privadas\n"
                            + "Ejemplo:     java DSA.java -key keys\n";
                    break;                
                case "-securitysign -h":
                    mensaje = "----------------------------------------------------------------------------------------------------------------------------------------------\n"
                   ...
                   case "-sign -h":
                    mensaje = "----------------------------------------------------------------------------------------------------------------------------------------------\n"
                            + "                                                 FIRMA ARCHIVO ALGORITMO DSA\n"
                            + "----------------------------------------------------------------------------------------------------------------------------------------------\n"
                            + "Esta es la segunda parte del algoritmo DSA, firmar el archivo este algoritmo generara el archivo de firma .sign a partir\n"
                            + "de una llave privada. Los parametros para usar este algoritmo son la llave privada, el archivo a firmar.\n"
                            + "Este metodo generará dos archivos S.sign y R.sign para la composición de la firma digital.\n\n"
                            + "Sintaxis: java DSA.java -sign <LlavePrivada> <ArchivoAFirmar> \n"
                            + "<LlavePrivada>:      Archivo de Llave Privada.\n"
                            + "<ArchivoAFirmar>     Archivo para firmar digitalmente.\n"
                            + "Ejemplo:     java DSA.java -ysign key.private Archivo.txt \n";
                    break;        
                ...
                case "-verify -h":
                    mensaje = "---------------------------------------------------------------------------------------------------------------------------------------------\n"
                            + "                                                 VERIFICAR FIRMA DIGITAL ALGORITMO DSA\n"
                            + "---------------------------------------------------------------------------------------------------------------------------------------------\n"
                            + "Esta es la tercera parte del algoritmo DSA verificar la autenticidad de un archivo. Con la firma digital se certifica un documento\n"
                            + "y se le pone una marca de tiempo. Si el documento de manera posterior a la firma es editado, cualquier intento por verificar la firma fallaria.\n\n"
                            + "Sintaxis: java DSA.java -verify <LlavePublica> <ArchivoAVerificar> <FirmaDigital>\n"
                            + "<LlavePublica>:      Archivo de Llave Privada.\n"
                            + "<ArchivoAVerificar>  Archivo a verificar la autenticidad con la firma digital.\n"
                            + "<R.sign>       Archivo Rd e firma digital.\n\n"
                            + "<S.sign>       Archivo S de firma digital.\n\n"
                            + "Ejemplo:     java DSA.java -veryfy key.public Archivo.txt R.sig S.sign";
                    break;            
                default:
                    break;
